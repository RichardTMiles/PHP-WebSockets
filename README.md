...
### CLI Server caviates 
To setup this process in an Apache WebServer you will need the `mod_proxy` and `mod_proxy_wstunnel` Apache modules installed. You can then add the following directive to your `.htaccess` files. **Note:** htaccess overrides will need to be enabled.
...
- WebSockets create a single persistent duplex connection from a single client to a single server. What is the proper way to handle auto-scaling events, where multiple servers may handle a connection based on a load ballancer? When a user needs to update everone they may need to communicate across servers. You could perhaps default to locality which presents intresting use cases.
    - A user can be logged in and using their phone and browser at the same time? Ensuring unique idetifiers like `session_id` can help target an individual device. In our example we use [`posix_mkfifo`](https://www.php.net/manual/en/function.posix-mkfifo.php). If you had a single server operating your site you could target a user using [`glob`](https://www.php.net/manual/en/function.glob) if the `FIFO` was named something like this example `session_id() . '_' . $_SESSION['id'] . '.fifo'`. **Note:** `$_SESSION['id']` is the users unique id stored in the database.
- Memory managment is critical, you should only process what is needed on the main thread. Forking when actually processing data is a very good idea. This increases the chances your server stays running verses failing due to an unexpected condition in one users experece. You Dev-Ops experence, future you, will be greatful.
- Everything that can fail will fail. Everything. From system `select` calls using PHP's `stream_select` to simple `print` statments. This typically happens with [poor system interrupt's](https://en.wikipedia.org/wiki/Interrupt) which happen ALL THE TIME or maybe, much less commonly, a stray photon shot from the sun hitting your server in the perfect spot at the right time.
    - Check every return value of anything that gives a retun no matter how consistently it seems to work. Log clear appropriate throwables to help debug when the **crazy** happens.
    - Keep your server start script managed by an OS level process scheduler so when it does fail, you can recover (start a new process) immediately
- If you use the `PHP-CLI` implementation then a `systemctl` process or similar system level porcess manager should be used to ensure the server is **always running**. The `PHP-CGI` implementation does not need this.
- You can choose wheater to handle all connections in a single thread `CLI`, or use `CGI` to have a single thread per user. It is currently unclear as to the best implementation, but specific use cases may need to choose one or the other. The immediate trade off is the convience of sending global updates which is easier in `CLI`, or speed which **could** potentially be faster with `CGI` though this depends on hardware specification and load.
