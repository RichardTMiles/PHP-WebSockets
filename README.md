...
- WebSockets create a single persistent duplex connection from a single client to a single server. What is the proper way to handle auto-scaling events, where multiple servers may handle a connection based on a load ballancer? When a user needs to update everone they may need to communicate across servers. If it was configured and appropreate, you could default to locality with presents intresting use cases.
