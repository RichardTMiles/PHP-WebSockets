...
- There is a max limit of file descriptors (fd) your computer can have open and a max open fd any process can have open; this is typically determined by the operating system's limit on file descriptors for a process. Any client connection is a FD socket. Any running process typically has few file descriptors that are automatically opened: Standard Input (`stdin`): Typically FD `0`, Standard Output (`stdout`): Typically FD `1`, Standard Error (`stderr`): Typically FD `2`. On linux (Mac, Ubuntu, fedora, RHEL,...) systems you can see limits set using `ulimit -a` and specifically file descriptors with `ulimit -n`. Natably
