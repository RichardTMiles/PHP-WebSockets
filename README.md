...
- Websockets create a single persistent connection to a single server. What is the proper way to handle auto-scaling events, where multiple servers may handle a connection based on a load ballancer. When a user needs to update everone they may need to communicate across servers. If it was configured and appropreate, you could default to locality with presents intresting use cases.
- Memory managment is critical, you should only process what is needed on the main thread. Forking when actually processing data is a very good idea. This increases the chances your server stays running versing failing because of an unexpected condition in one users experece.
